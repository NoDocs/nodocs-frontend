...
export const transMergeNode = (leftOp, rightOp) => {
  switch (rightOp.type) {
    case 'insert_text': {
    ...
    case 'insert_node': {
      if (Path.equals(leftOp.path, rightOp.path)) {
        ...
        return [
          leftOp,
          { ...leftOp, position: leftOp.position + offset },
        ]
      }
    ...
    case 'split_node': {
      if (Path.equals(leftOp.path, rightOp.path)) {
        return [
          leftOp,
          { ...leftOp, position: leftOp.position + rightOp.position },
        ]
      }
      ...
      if (path[path.length - 1] === 0) {
        return [
          { ...rightOp, type: 'merge_node', path: Path.next(rightOp.path) },
          leftOp,
        ]
      }
